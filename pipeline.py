...
def format_timestamp(seconds: float) -> str:
    """Utility to convert seconds into MM:SS format for UI readability."""
    return f"{int(seconds // 60):02d}:{int(seconds % 60):02d}"

class Pipeline:
    """
    Main processing engine for Lumina Audit Assistant.
    Orchestrates transcription, RAG indexing (with batch processing for stability),
    and visual analysis. Optimized for macOS hardware constraints.
    """
    ...
    def __init__(self, data_dirs, whisper_model="base"):
        ...
        self.whisper_engine = WhisperTranscriber(self.DATA_DIRS["transcripts"], model_name=whisper_model)
        self.vector_store = LocalVectorStore()
        self.insight_engine = VideoInsight()

        # Data processor for structured JSON handling
    ...
    def run(self, source_type, source_value, duration=30, use_vision=True):
        ...
        Sequence: 1. Data Intake -> 2. Whisper Transcription -> 3. FAISS Indexing -> 4. Vision
        """
        try:
            if not source_value:
                return None
            ...
            # --- 3. RAG INDEXING (Optimized with Batching) ---
            # We process in small chunks to prevent FAISS/Memory crashes on Mac.
    ...
    def prepare_export(self, source_id: str):
        """Generates the final audit ZIP package."""
    ...
    def get_active_sources(self) -> list:
        """Retrieves list of successfully processed audit sources."""
        processed_dir = self.DATA_DIRS["processed"]
        return [f.name.replace("_processed.json", "") for f in processed_dir.glob("*_processed.json")]

    def get_human_readable_sources(self) -> dict:
        """Maps file IDs to user-friendly titles for the UI."""
    ...
    def render_transcript_view(self, source_id: str, T: dict):
        """Streamlit UI component for transcript and visual proof review."""
        p_file = self.DATA_DIRS["processed"] / f"{source_id}_processed.json"
        frame_dir = self.DATA_DIRS["frames"] / source_id
        ...
        try:
            if vision_enabled and has_frames:
                anchored_data = linker.get_anchored_frames(segments)
            else:
                anchored_data = [{"start": s['start'], 'audio_track': s['text'], "frame": None} for s in segments]
        except:
            anchored_data = [{"start": s['start'], 'audio_track': s['text'], "frame": None} for s in segments]
    ...
    def _register_source(self, source_id, source_title):
        """Logs processed sources into the central registry for traceability."""
        REGISTRY_FILE = self.DATA_DIRS["raw"] / "registry.json"
        registry = {}
        if REGISTRY_FILE.exists():
            with open(REGISTRY_FILE, "r", encoding="utf-8") as f:
